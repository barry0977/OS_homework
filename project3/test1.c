...
#define RAMFS_FILE "../mnt/ramfs_sync/testfile_single.dat"
...
int main() {
    ...
    for (int written = 0; written < total_size; written += chunk) {
        ...
        //     ssize_t now = write(fd, buf + total_written, chunk - total_written);
        //     if (now < 0) {
        //         perror("write failed");
        //         break;
        //     } else if (now == 0) {
        ...
        //     }
        // }
        // if (total_written != chunk) {
        //     fprintf(stderr, "Partial write: %zd/%d bytes written\n", total_written, chunk);
        //     exit(1);
        // }
